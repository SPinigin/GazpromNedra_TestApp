...
class LicenseRepository:
    ...
    def get_by_id(self, license_id: uuid.UUID):
    ...
    def get_by_status(self, status_id: uuid.UUID, skip: int = 0, limit: int = 100, sort_by: Optional[str] = None,
                      order: str = "asc"):
    ...
    def update(self, license_id: uuid.UUID, license_update: LicenseUpdate):
    ...
    def delete(self, license_id: uuid.UUID):
        db_license = self.db.query(License).filter(License.id == license_id).first()
        if db_license:
